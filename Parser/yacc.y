...
global : type ID OPEN_PARENTHESES arguments CLOSE_PARENTHESES DOT global	{
			$$ = newNode("global");
			append($$, $1);
...
type :	T_INT {
			$$ = newNodeWithValue(NULL, tintN);
		} |	T_STRING {
			$$ = newNodeWithValue(NULL,tstringN);
		} |	T_CHAR {
			$$ = newNodeWithValue(NULL, tcharN);
		} |	T_DECIMAL {
			$$ = newNodeWithValue(NULL, tdecimalN);
		} |	T_VOID {
			$$ = newNodeWithValue(NULL, tvoidN);
};
...
declare	: type ID {
        ...
        append($$, $5);
} |	type CONSTANT ID ASSIGN STRING {
		$$ = newNode("declare");
        append($$, $1);
        append($$, newNodeWithValue(NULL, constantN));
        append($$, newNodeWithValue($3, idN));
...
assval : ASSIGN	{
		...
		$$ = newNodeWithValue(NULL, sumAssignN);
} |	SUB_ASSIGN {
		$$ = newNodeWithValue(NULL, subAssignN);
} |	MUL_ASSIGN {
		$$ = newNodeWithValue(NULL, mulAssignN);
} |	DIV_ASSIGN	{
		$$ = newNodeWithValue(NULL, divAssignN);
} |	OP_ASSIGN	{
...
when : WHEN OPEN_PARENTHESES condition CLOSE_PARENTHESES OPEN_CURLY_PARENTHESES code CLOSE_CURLY_PARENTHESES else {
		$$ = newNode("when");
		...
		append($$, $3);
		append($$, newNodeWithValue(NULL, clCurlyN));
} |	ELSE WHEN OPEN_PARENTHESES condition CLOSE_PARENTHESES OPEN_CURLY_PARENTHESES code CLOSE_CURLY_PARENTHESES else	{
		$$ = newNode("else");
		append($$, newNodeWithValue(NULL, elseN));
		append($$, newNodeWithValue(NULL, whenN));
		append($$, newNodeWithValue(NULL, opParenthesesN));
...
during : REPEAT OPEN_CURLY_PARENTHESES code CLOSE_CURLY_PARENTHESES DURING OPEN_PARENTHESES condition CLOSE_PARENTHESES DOT	{
		$$ = newNode("during");
		...
		append($$, newNodeWithValue(NULL, clParenthesesN));
		append($$, newNodeWithValue(NULL, dotN));
} |	DURING OPEN_PARENTHESES condition CLOSE_PARENTHESES OPEN_CURLY_PARENTHESES code CLOSE_CURLY_PARENTHESES	{
		$$ = newNode("during");
		append($$, newNodeWithValue(NULL, duringN));
		append($$, newNodeWithValue(NULL, opParenthesesN));
		append($$, $3);
...
compare	: EQUAL	{
	$$ = newNodeWithValue(NULL, equalN);
} |	NOT_EQUAL {
	$$ = newNodeWithValue(NULL, notEqualN);
} |	LEQ_THAN {
	$$ = newNodeWithValue(NULL, leqN);
} |	GEQ_THAN {
	$$ = newNodeWithValue(NULL, geqN);
} |	LESS_THAN {
	$$ = newNodeWithValue(NULL, lessN);
} |	GREATER_THAN {
	$$ = newNodeWithValue(NULL, greaterN);
};
